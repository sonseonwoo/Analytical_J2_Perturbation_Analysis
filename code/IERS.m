...
if (interp =='l')
    ...
    x_pole  = x_pole/C.Arcs;  % Pole coordinate [rad]
    y_pole  = y_pole/C.Arcs;  % Pole coordinate [rad]
    dpsi    = dpsi/C.Arcs;    % [rad]
    deps    = deps/C.Arcs;    % [rad]
    dx_pole = dx_pole/C.Arcs; % Pole coordinate [rad]
    dy_pole = dy_pole/C.Arcs; % Pole coordinate [rad]
elseif (interp =='n')    
    mjd = floor(Mjd_UTC);
    i = find(mjd==eop(4,:),1,'first');
    eop = eop(:,i);
    % Setting of IERS Earth rotation parameters
    % (UT1-UTC [s], TAI-UTC [s], x ["], y ["])
    x_pole  = eop(5)/C.Arcs;  % Pole coordinate [rad]
    y_pole  = eop(6)/C.Arcs;  % Pole coordinate [rad]
	UT1_UTC = eop(7);             % UT1-UTC time difference [s]
    LOD     = eop(8);             % Length of day [s]
    dpsi    = eop(9)/C.Arcs;  % [rad]
    deps    = eop(10)/C.Arcs; % [rad]
    dx_pole = eop(11)/C.Arcs; % Pole coordinate [rad]
    dy_pole = eop(12)/C.Arcs; % Pole coordinate [rad]
