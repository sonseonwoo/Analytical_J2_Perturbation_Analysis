...
for k = 1:num_steps
    nu_scalar = M2nu(M_hist(k), e_hist(k)); 
    nu_hist(k) = nu_scalar;
...
for k = 1:num_steps
    ...
    % [PDF: 1590-1618, 1624-1628, 1633-1638, 1641-1658]
    ...
    delta_a_total = (J2 * R_e^2 / a_k) * ...
        ...
        * (3 * sin(i_k)^2 / 2) );
    % 
    % [cite_start]% [주의] de_total은 제공된 코드를 그대로 사용, PDF 수식 [cite: 1603-1620]과 다를 수 있음
    ...
    % [수정] nu - M + e*sin(nu)는 M = E - e*sin(E), nu=... 관계에서 E - M 입니다.
    %        M_k (평균)와 nu_k (평균)를 사용합니다.
    E_k = M2E(M_k, e_k); % M_k로부터 E_k를 구하는 함수가 필요합니다. M2nu 내부 로직 재활용.
    v_M_diff = E_k - M_k; % (nu - M + e*sin(nu)) 항은 (E - M)과 같습니다.
    ...
    % [PDF: 1599-1658] (단, k번째 omega_k 사용)
    
    delta_a_mean = 0; 

    mean_e_factor = (1/16) * (J2 * R_e^2 / p_k^2) * sin(i_k)^2 * ((1-e_k^2)/e_k) * cos(2*omega_k);
    ...
    delta_M_mean = -(3 * J2 * R_e^2 / p_k^2) * sqrt(1 - e_k^2) * sin(i_k)^2 * term1_M * sin(2*omega_k); 
